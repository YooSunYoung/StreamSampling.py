...
class WeightedReservoirSamplerMulti:
        ...
        if self.seen_k <= self.n:
            ...
            if self.seen_k == self.n:
                ...
                indices = self.rng.choice(
                    self.n, size=self.n, replace=True, p=self.weights / self.state
                )
    ...
    def ordered_value(self) -> List[Any]:
        ...
        if not self.ordered:
            raise ValueError(
                "Ordered sampling was not enabled. Set ordered=True during initialization."
            )
...
if __name__ == "__main__":
    ...
    def test_algorithms():
        ...
        weights = [
            1.0 if x <= 5 else 2.0 for x in elements
        ]  # 1-5: weight=1, 6-10: weight=2
        n_sample = 3
        n_trials = 5000

        methods = [(SamplingMethod.WRSWR_SKIP, "WRSWR-SKIP")]
    ...
    def test_streaming():
        ...
        for element, weight in stream_elements:
            ...
            current_sample = sampler.value()
            print(
                f"  Added {element} (weight={weight:.3f}), current sample: {current_sample}"
            )
    ...
    def test_ordered_sampling():
        ...
        sample_ordered = weighted_sample_multi(
            elements, weights, n=3, ordered=True, rng=np.random.default_rng(42)
        )

        # Test with ordered=False
        sample_unordered = weighted_sample_multi(
            elements, weights, n=3, ordered=False, rng=np.random.default_rng(42)
        )
